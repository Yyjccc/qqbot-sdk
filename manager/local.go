...
type ChanManager struct {
	sessionChan chan websocket.Session
}

// Start 启动本地 session manager
func (l *ChanManager) Start(apInfo *websocket.WebsocketAP, token *entry.Token, intents *websocket.Intent) error {
	defer util.Sync()
	if err := CheckSessionLimit(apInfo); err != nil {
		util.Errorf("[ws/session/local] session limited apInfo: %+v", apInfo)
		return err
	}
	startInterval := CalcInterval(apInfo.SessionStartLimit.MaxConcurrency)
	util.Infof("[ws/session/local] will start %d sessions and per session start interval is %s",
		apInfo.Shards, startInterval)

	// 按照shards数量初始化，用于启动连接的管理
	l.sessionChan = make(chan websocket.Session, apInfo.Shards)
...
func (l *ChanManager) newConnect(session websocket.Session) {
	defer func() {
	...
	if err != nil {
		util.Errorf("[ws/session] Identify/Resume err %+v", err)
		return
	}
	if err := wsClient.Listening(); err != nil {
		util.Errorf("[ws/session] Listening err %+v", err)
